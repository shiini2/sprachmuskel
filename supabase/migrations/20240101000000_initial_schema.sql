-- Initial schema for Sprachmuskel
...
-- Daily sessions for streak tracking
...
CREATE POLICY "Users can manage own exercise history" ON exercise_history FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Users can manage own daily sessions" ON daily_sessions FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Users can manage own seen sentences" ON seen_sentences FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "Users can manage own tutor messages" ON tutor_messages FOR ALL USING (auth.uid() = user_id);

CREATE POLICY "Anyone can view grammar topics" ON grammar_topics FOR SELECT TO PUBLIC USING (true);
...
-- Indexes
CREATE INDEX IF NOT EXISTS idx_exercise_history_user_date ON exercise_history(user_id, created_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_tutor_messages_user_date ON tutor_messages(user_id, created_at DESC);
